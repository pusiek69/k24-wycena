...
function nowaStrona(m) {
  ...
  const podmiany = [
    ...
    [`<title>Blaty kuchenne ${WZ.nazwa} — kamień na wymiar</title>`,
     `<title>Blaty kuchenne ${m.nazwa} — kamień na wymiar</title>`],
    [`content="Blaty kuchenne — ${WZ.nazwa}"`, `content="Blaty kuchenne — ${m.nazwa}"`],
    [`blaty-kuchenne-${WZ.slug}`, `blaty-kuchenne-${m.slug}`],
    // ── nazwa miasta w danych strukturalnych i treści ────────────────
    [`"name": "Blaty kuchenne ${WZ.nazwa} — kamień na wymiar"`,
     `"name": "Blaty kuchenne ${m.nazwa} — kamień na wymiar"`],
    // Okruszek i areaServed usługi są łamane na wiele linii — idą regexem niżej.
    [`"name": "Blaty kuchenne na wymiar — ${WZ.nazwa}"`,
  ...
  // Okruszek i pojedyncze areaServed usługi — JSON-LD łamany na linie.
  t = t.replace(
    new RegExp(`("position": 2,\\s*"name": ")${WZ.nazwa}(",\\s*"item")`),
    `$1${m.nazwa}$2`
  );
  t = t.replace(
    new RegExp(`("areaServed": \\{\\s*"@type": "City",\\s*"name": ")${WZ.nazwa}(")`),
    `$1${m.nazwa}$2`
  );

  t = t.replace(/(name="description" content=")[^"]*(")/, `$1${opisDojazdu}$2`);
  t = t.replace(/(property="og:description" content=")[^"]*(")/, `$1${opisDojazdu}$2`);
  t = t.replace(/("description": ")Blaty kamienne[^"]*(")/, `$1${opisDojazdu}$2`);
  t = t.replace(/<p class="sub">[\s\S]*?<\/p>/, `<p class="sub">${m.krotki}</p>`);
  t = t.replace(/(<h2>[^<]*i okoliczne miejscowości<\/h2>\s*<p>)[\s\S]*?(<\/p>)/, `$1\n        ${akapitDojazdu}\n      $2`);
...
MIASTA.forEach((m, i) => {
  ...
  // Schema — przed zamknięciem <head>.
  const reS = /  <script type="application\/ld\+json">\s*\{\s*"@context"[^]*?"@type": "FAQPage"[^]*?<\/script>/;
  t = reS.test(t) ? t.replace(reS, schema) : t.replace(/(\s*)<\/head>/, `\n${schema}\n$1</head>`);
